...
import type { PaymentLinkLine } from "@foodo/utils";
...
export interface PreparedPaymentLink { token: string; customerName: string; items: PaymentLinkLine[]; retrying?: boolean }

export default function CheckoutClient({ preparedLink }: { preparedLink?: PreparedPaymentLink }) {
  const router = useRouter();
  const { restaurant } = useRestaurant();
  const cartItems = useCartStore((s) => s.items);
  const items = useMemo(() => preparedLink ? preparedLink.items.map((item, index) => ({
    ...item, price: item.priceKobo, lineTotal: item.priceKobo * item.quantity,
    imageUrl: undefined as string | undefined, optionsKey: `prepared-${index}`,
  })) : cartItems, [preparedLink, cartItems]);
  const subtotal = items.reduce((sum, item) => sum + item.lineTotal, 0);
  const clearStoreCart = useCartStore((s) => s.clear);
  const clearCart = () => { if (!preparedLink) clearStoreCart(); };
  ...
  const [firstName, setFirstName] = useState(preparedLink?.customerName.split(" ")[0] ?? "");
  const [lastName, setLastName] = useState(preparedLink?.customerName.split(" ").slice(1).join(" ") ?? "");
  const [email, setEmail] = useState("");
  const [step, setStep] = useState<"phone" | "checkout">(preparedLink ? "checkout" : "phone");
  ...
  async function handlePay() {
    ...
    try {
      ...
      const res = await fetch("/api/checkout/initialize", {
        ...
        body: JSON.stringify({
          restaurantId: restaurant.id,
          paymentLinkToken: preparedLink?.token,
      ...
      if (!res.ok) {
        ...
        if (!preparedLink && Array.isArray(initData.unavailableItemIds)) {
        ...
        if (preparedLink && initData.reopenPaymentLink) {
          window.location.reload();
          return;
        }
      ...
      if (preparedLink && initData.checkoutUrl) {
  ...
  return (
    <div className="min-h-screen bg-black-50 pb-32">
      {preparedLink && <div className="bg-primary/10 px-5 py-4 text-center text-sm text-black-800">{preparedLink.retrying
        ? <>Your last payment didn’t go through, so nothing was charged. Your order is still here — check your details and try again.</>
        : <>Your order has been prepared by {restaurant.name}. Confirm your details below. Need to change a meal? Ask the restaurant for a new link.</>}</div>}
          ...
          <div className="bg-white rounded-2xl border border-black-100 p-6 space-y-5 w-full max-w-sm shadow-xl">
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-black-500">
                {preparedLink ? "Your order is ready. Enter your phone number to continue." : "Enter your phone number to continue"}
              </p>
            </div>
